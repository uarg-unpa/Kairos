...
# Grafico de estimaciones
estimaciones = np.arange(1, 8)
optimista = [2.0, 1.9, 3.6, 3.2, 2.9, 1.7, 0.7]
probable  = [2.3, 2.9, 6.8, 5.1, 4.6, 2.2, 1.2]
pesimista = [2.6, 4.1, 7.3, 6.9, 6.2, 3.5, 1.6]
real      = [None, None, None, None, None, 1.8, 0.7]
...
ax.plot(estimaciones, probable, color="#e60073", marker="o", label="Mas probable", linewidth=2)
ax.plot(estimaciones, pesimista, "r--o", label="Pesimista")
ax.plot([6,7], [real[5], real[6]], "o-", color="#0073e6", label="Real", linewidth=2.3)

for i, val in enumerate(probable):
    ax.text(i+1, val+0.25, f"{val:.1f} m", ha='center', fontsize=9, color='black')

ax.set_title("Grafico de estimaciones", fontsize=13, fontweight='bold', color='#e60073')
ax.set_xlabel("Numero de estimacion")
ax.set_ylabel("Duracion estimada (meses)")
...
# Grafico estimaciones (lineal con fechas)
...
optimista = [2.0, 1.9, 3.6, 3.2, 2.9, 1.7, 0.7]
caso_probable  = [2.3, 2.9, 6.8, 5.1, 4.6, 2.2, 1.2]
pesimista = [2.6, 4.1, 7.3, 6.9, 6.2, 3.5, 1.6]
real      = [None, None, None, None, None, 1.8, 0.7]
...
series = [
    ...
    (fechas_cen, "Mas probable", "#e60073"),
...
ax.set_yticklabels(["Optimista", "Mas probable", "Pesimista", "Real"])
ax.set_xticks(estimaciones)
ax.set_xlim(0.5, len(estimaciones) + 0.5)
ax.set_xlabel("Numero de estimacion")
ax.set_title("Linea de tiempo de fechas de finalizacion", fontsize=13, fontweight='bold', color='#e60073')
...
# Grafico de comparacion en horas hombre

hh_opt = [600, 570, 637.4, 611, 640, 366.6, 162.9]
hh_cen = [690, 870, 1199.7, 1187.9, 1005.2, 483.3, 255.9 ]
hh_pes = [780, 1229, 1616.5, 1371.3, 1355, 776.6, 345.1]
hh_real = [None, None, None, None, None, 387.4, 204.7]

labels = [f"E{i}" for i in range(1, 8)]
x = np.arange(len(labels))
width = 0.2

fig, ax = plt.subplots(figsize=(10,6))
ax.bar(x - 1.5*width, hh_opt, width, label='Optimista', color='lightgreen')
ax.bar(x - 0.5*width, hh_cen, width, label='Mas probable', color='pink')
...
ax.set_title("Comparacion de esfuerzo (h-h) por estimacion", fontsize=13, fontweight='bold', color='#e60073')
