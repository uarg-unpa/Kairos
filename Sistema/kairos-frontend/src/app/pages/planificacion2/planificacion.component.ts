...
export class PlanificacionComponent implements OnInit {
  ...
  cargarEtapas(): void {
    if (!this.proyectoId) {
      console.warn('No hay proyectoId definido, no se pueden cargar las etapas');
      return;
    }
...
guardarCategoriaEditada(): void {
  ...
  agregarTarea(): void {
    ...
    const fechaInicioIteracion = new Date(this.iteracionActual?.fechaInicio || '');
    const fechaFinIteracion = new Date(this.iteracionActual?.fechaFin || '');
    ...
    if(fechaInicio < fechaInicioIteracion || fechaFin > fechaFinIteracion) {
      alert(`⚠️ Las fechas deben estar dentro del rango de la iteración actual: ${this.iteracionActual?.fechaInicio} a ${this.iteracionActual?.fechaFin}.`);
      return;
    }
