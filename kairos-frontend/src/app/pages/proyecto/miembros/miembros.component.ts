...
export class MiembrosComponent implements OnInit {
  ...
  cerrarModalAgregar(): void {
    ...
    buscarUsuarios(): void {
        ...
        this.usuariosService.searchByName(this.searchQuery).subscribe({
            next: (usuarios) => {
            // Filtrar los que ya están en el proyecto
            const miembrosIds = this.miembros.map(m => m.idUsuario);
            this.usuariosBusqueda = usuarios.filter(u => !miembrosIds.includes(u.id));
            },
            error: (err) => {
  ...
  agregarMiembro(idUsuario: number | null): void {
    if (idUsuario) {
      this.proyectoService.agregarMiembro(this.proyectoId!, idUsuario, this.newRol).subscribe({
        next: () => {
      ...
      this.proyectoService.invitarMiembro(this.proyectoId!, this.newEmail, this.newRol).subscribe({
        ...
        error: () => this.errorMensaje = 'Error al invitar miembro'
      });
    } else {
  ...
  invitarMiembro(): void {
    if (!this.newEmail || !this.proyectoId) {
    ...
    this.proyectoService.invitarMiembro(this.proyectoId, this.newEmail, this.newRol).subscribe({
        ...
        alert('Error: ' + (err.error || 'No se pudo invitar'));
        }
    });
    }
